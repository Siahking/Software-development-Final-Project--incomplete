import { getConstraints,findWorker,deleteConstraints,createConstraint,editConstraints } from "../../static/javascript/backend.js";
...
export async function displayConstraints(){
    ...
    for (const constraint of results){
        ...
        const worker1 = await findWorker("","","","",worker1Id)
        const worker2 = await findWorker("","","","",worker2Id)
...
export async function changeConstraint(event){
    ...
    const result = await editConstraints(constraintId.value,worker1IdInput.value,worker2IdInput.value)
